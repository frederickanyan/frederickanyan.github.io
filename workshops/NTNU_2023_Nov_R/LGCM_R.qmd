...
Throughout this workshop, we will use only one function from the lavaan package. So, feel at ease even if it is your first time working in R.
We will do several chi-square difference tests to select best fitting models during the workshop.
:::
...
data <- read_sav("/Volumes/anyan/Quant_Resources/0000 Workshops/000_Nov_NTNU/data.sav")#Path and data

library(lavaan)#We will use the growth() function in the lavaan pacakage, which also has other functions. [See here](https://lavaan.ugent.be)
```

```{r}
#pairs.panels(data[,c("lone1", "lone2", "lone3", "lone4", "lone5")])

#Longitudinal plot
#ggplot(data = datalong, aes(x = year, y = lone, group = personid)) +
#  geom_point() + 
#  geom_line() +
#  xlab("Year") + 
#  ylab("Loneliness") + ylim(1,6) +
...
#Estimate the intercept of subs
subs5 ~ 1'
...
## Model 11
```{r}
# 009_Multigroup LGCM M1 Invariance model - M1
...
## Model 12
```{r}
# 009_Multigroup LGCM M2 Free factor means - M2
...
## Model 13
```{r}
# 009_Multigroup LGCM M3 Free factor means and co/variances - M3
...
## Model 14
```{r}
# 009_Multigroup LGCM M4 Free factor means and co/variances - M4
...
lone1 ~~ c(resm1, resf1)*lone1
lone2 ~~ c(resm2, resf2)*lone2
lone3 ~~ c(resm3, resf3)*lone3
lone4 ~~ c(resm4, resf4)*lone4
lone5 ~~ c(resm5, resf5)*lone5
'
...
# 010_Parallel growth curve models - anxiety and depression symptoms
...
## Model 16
```{r}
# 010_Parallel growth curve models - anxiety and depression symptoms with autocorrelations
...
## Model 17
```{r}
# 010_Parallel growth curve models - anxiety and depression symptoms with cross-domain correlations 
...
## Model 18
```{r}
# 011_ Sequentially contigent growth curve model anxiety t1-t5 predicting depression t6-t10
