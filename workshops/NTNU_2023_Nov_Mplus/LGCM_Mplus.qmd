...
The Mplus syntax provided here are for those using Mplus, although the syntax can be modified for use in R through Mplus Automation.
:::

::: {.callout-warning appearance="simple"}
I have included LISTWISE = ON under the DATA command to match the results from Mplus to the output from R since this workshops was taught along with R. You do not have to use listwise deletion in your own application. FIML is better and it is the default in Mplus.
:::
...
MODEL:
  ...
  !Sequentially testing models means specific parameters are of interest
  !For those parameters, we will begin by constraining them to be equal (identical) across b
